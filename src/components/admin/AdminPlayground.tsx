...
import { ArrowLeft, RotateCcw, Trash2, Send, Loader2, Server, BookOpen, Sliders, ChevronDown, ChevronRight, Brain, Plus, X } from 'lucide-react';
...
export default function AdminPlayground() {
  ...
  const addCustomSkill = () => {
    ...
    pg.setConfig({ customSkills: [...pg.config.customSkills, { name, content }] });
    setCustomSkillName('');
  ...
  const removeCustomSkill = (name: string) => {
    pg.setConfig({ customSkills: pg.config.customSkills.filter((s) => s.name !== name) });
  };
  ...
  const addCustomMcpTool = () => {
    ...
    try { parameters = JSON.parse(customMcpSchema); } catch { return; }
    pg.setConfig({ customMcpTools: [...pg.config.customMcpTools, { name, description, parameters }] });
    setCustomMcpName('');
    setCustomMcpDesc('');
    setCustomMcpSchema('{\n  "type": "object",\n  "properties": {}\n}');
  ...
  const removeCustomMcpTool = (name: string) => {
    pg.setConfig({ customMcpTools: pg.config.customMcpTools.filter((t) => t.name !== name) });
  };

  const effectivePreview = `${BASE_SYSTEM_PROMPT}${pg.config.customSystemPrompt ? '\n\n' + pg.config.customSystemPrompt : ''}${pg.config.enableSmartThinking ? '\n\n[CRITICAL: think step by step inside <reason> tags]' : ''}\n[+ enabled skill + tool guardrail blocks added server-side]`;
    ...
    <div className={`min-h-screen ${theme.background} ${theme.text} flex flex-col`}>
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full px-4 py-4 grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-4">
        ...
        <aside className="space-y-4 lg:max-h-[calc(100vh-7rem)] lg:overflow-y-auto pr-1 custom-scrollbar">
          ...
          {/* Smart Thinking toggle */}
          <GlassPanel className="p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-xs uppercase tracking-widest text-white/40">
                <Brain className="w-3.5 h-3.5" /> Smart thinking
              </div>
              <button
                onClick={() => pg.setConfig({ enableSmartThinking: !pg.config.enableSmartThinking })}
                className={`relative w-10 h-5 rounded-full transition-colors ${pg.config.enableSmartThinking ? 'bg-purple-500' : 'bg-white/15'}`}
              >
                <span className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white transition-transform ${pg.config.enableSmartThinking ? 'translate-x-5' : 'translate-x-0'}`} />
              </button>
            </div>
          ...
          <GlassPanel className="p-4">
            ...
            {skillsOpen && (
              <div className="mt-3 space-y-1.5">
                {/* Custom skills list */}
                {pg.config.customSkills.map((s) => (
                  <div
                    key={s.name}
                    className="flex items-center justify-between px-2.5 py-2 rounded-lg border border-amber-400/40 bg-amber-500/10 text-sm"
                  >
                    <div className="truncate">
                      <span className="font-medium">{s.name}</span>
                      <p className="text-[10px] text-white/40 truncate">{s.content.slice(0, 80)}</p>
                    </div>
                    <button onClick={() => removeCustomSkill(s.name)} className="text-white/30 hover:text-red-400 ml-1">
                ...
                ) : pg.config.customSkills.length === 0 ? (
                  <p className="text-xs text-white/30">No skills in catalog. Add one below.</p>
                ) : null}

                {/* Add custom skill button / form */}
                {showCustomSkillForm ? (
                  <div className="mt-2 space-y-2 border border-dashed border-white/15 rounded-lg p-2">
                    <input
                      value={customSkillName}
                      onChange={(e) => setCustomSkillName(e.target.value)}
                      placeholder="Skill name"
                      className="w-full bg-white/5 border border-white/10 rounded px-2 py-1.5 text-xs outline-none focus:border-amber-400/50"
                    />
                    <textarea
                      ...
                      rows={3}
                      className="w-full bg-white/5 border border-white/10 rounded px-2 py-1.5 text-xs outline-none focus:border-amber-400/50 resize-y font-mono leading-relaxed"
                    />
                    <div className="flex gap-2">
                      <button onClick={addCustomSkill} className="flex-1 px-2 py-1.5 rounded bg-amber-500/20 border border-amber-400/40 text-xs hover:bg-amber-500/30">
                        Add
                      </button>
                      <button onClick={() => setShowCustomSkillForm(false)} className="px-2 py-1.5 rounded bg-white/5 border border-white/10 text-xs hover:bg-white/10">
                        Cancel
                      </button>
                    </div>
                  </div>
                ) : (
                  <button onClick={() => setShowCustomSkillForm(true)} className="mt-2 w-full flex items-center justify-center gap-1.5 px-2.5 py-2 rounded-lg border border-dashed border-white/10 text-xs text-white/40 hover:text-white/70 hover:border-white/20">
          ...
          <GlassPanel className="p-4">
            ...
            {mcpOpen && (
              <div className="mt-3 space-y-1.5">
                ...
                {pg.config.customMcpTools.map((t) => (
                  <div
                    key={t.name}
                    className="flex items-center justify-between px-2.5 py-2 rounded-lg border border-cyan-400/40 bg-cyan-500/10 text-sm"
                  >
                    <div className="truncate">
                      <span className="font-medium">{t.name}</span>
                      <p className="text-[10px] text-white/40 truncate">{t.description}</p>
                    </div>
                    <button onClick={() => removeCustomMcpTool(t.name)} className="text-white/30 hover:text-red-400 ml-1">
                      <X className="w-3.5 h-3.5" />
                    </button>
                  </div>
                ))}
                ...
                ) : pg.config.customMcpTools.length === 0 ? (
                  <p className="text-xs text-white/30">No MCP servers published. Add one below.</p>
                ) : null}

                <p className="text-[10px] text-white/30 pt-1">Tools run inline (auto-approved) — admins trust themselves.</p>

                {/* Add custom MCP tool button / form */}
                {showCustomMcpForm ? (
                  <div className="mt-2 space-y-2 border border-dashed border-white/15 rounded-lg p-2">
                    <input
                      value={customMcpName}
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded px-2 py-1.5 text-xs outline-none focus:border-cyan-400/50"
                    />
                    <input
                      value={customMcpDesc}
                    ...
                    />
                    <textarea
                      value={customMcpSchema}
                      onChange={(e) => setCustomMcpSchema(e.target.value)}
                      placeholder='JSON schema for parameters'
                      rows={4}
                      className="w-full bg-white/5 border border-white/10 rounded px-2 py-1.5 text-xs outline-none focus:border-cyan-400/50 resize-y font-mono leading-relaxed"
                    />
                    <div className="flex gap-2">
                      <button onClick={addCustomMcpTool} className="flex-1 px-2 py-1.5 rounded bg-cyan-500/20 border border-cyan-400/40 text-xs hover:bg-cyan-500/30">
                        Add
                      </button>
                      <button onClick={() => setShowCustomMcpForm(false)} className="px-2 py-1.5 rounded bg-white/5 border border-white/10 text-xs hover:bg-white/10">
                        Cancel
                      </button>
                    </div>
                  </div>
                ) : (
                  <button onClick={() => setShowCustomMcpForm(true)} className="mt-2 w-full flex items-center justify-center gap-1.5 px-2.5 py-2 rounded-lg border border-dashed border-white/10 text-xs text-white/40 hover:text-white/70 hover:border-white/20">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4">
              {pg.messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center text-white/40">
                  <p className="max-w-sm text-sm">Uses the latest TimeMachine PRO model</p>
                </div>
              ) : (
                pg.messages.map((m) => (
                  <div key={m.id} className={`flex flex-col ${m.isAI ? 'items-start' : 'items-end'}`}>
                    {m.isAI && m.thinking ? (
                      <div className="max-w-[85%] mb-2 w-full">
                        <button
                          onClick={() => {/* toggle collapse — could add state but simple for now */}}
                          className="flex items-center gap-1.5 text-[11px] text-zinc-400 hover:text-zinc-300 mb-1"
                        >
                          <Brain className="w-3 h-3" />
                          Thought to provide a better answer
                        </button>
                        <div className="text-xs text-zinc-400 border-l-2 border-zinc-500/30 pl-3 py-1 bg-white/[0.02] rounded-r-lg">
                          <ReactMarkdown
