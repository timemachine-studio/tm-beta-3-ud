  ...
  const renderReplyPreview = () => {
    ...
    return (
      <div className={`flex items-start gap-2 mb-2 px-3 py-2 rounded-lg bg-white/5 border-l-2 border-purple-500/50 text-sm max-w-[85%] ${isOwnMessage ? 'ml-auto' : ''}`}>
        <CornerDownRight className="w-3 h-3 text-white/40 mt-1 shrink-0" />
      ...
      <AnimatePresence>
        {showActions && (
          ...
          >
            <button
              onClick={handleReply}
              className="p-1.5 rounded-md bg-black/40 backdrop-blur-xs hover:bg-black/60 text-white/50 hover:text-white transition-colors"
              title="Reply"
            >
                  ...
                  >
                    {QUICK_REACTIONS.map(emoji => (
                      <button
                        key={emoji}
                        onClick={(e) => handleReact(emoji, e)}
                        className="p-1.5 rounded-sm hover:bg-white/10 text-lg transition-colors"
                      >
                        {emoji}
                      </button>
                    ))}
