...
import { fetchAuthorizedMedia } from '../../services/media/authorizedMedia';
...
function MusicPlayerVariation({ parsedData, seed, personaColor, themeText, savedAudioUrl, savedImageUrl, onSaved }: MusicPlayerVariationProps) {
  ...
  // The generated bytes, fetched once with the user's token. Display, the
  // Supabase upload and the download all use these — the previous version
  // fetched the generation URL up to three times, and each was a new
  // generation on the provider (pre-launch-audit.md A.2/A.8).
  const generationAudioBlob = useRef<Blob | null>(null);
  const generationImageBlob = useRef<Blob | null>(null);
  ...
  useEffect(() => {
    ...
    if (savedAudioUrl && savedImageUrl) {
      queueMicrotask(() => {
        ...
        setIsPlaying(false);
        setLoadError(null);
        generationAudioBlob.current = null;
        generationImageBlob.current = null;
      });
    ...
    if (!user) {
      queueMicrotask(() => {
        ...
        setLoadError('Sign in to generate music.');
      });
      return () => { cancelled = true; };
    }
    ...
    (async () => {
      try {
        const [audioBlob, imageBlob] = await Promise.all([
          fetchAuthorizedMedia(audio),
          fetchAuthorizedMedia(image),
        ]);
        if (cancelled) return;
        generationAudioBlob.current = audioBlob;
        generationImageBlob.current = imageBlob;
        const audioObjectUrl = URL.createObjectURL(audioBlob);
        const imageObjectUrl = URL.createObjectURL(imageBlob);
        objectUrls.push(audioObjectUrl, imageObjectUrl);
        ...
        setAudioLoading(false);
        setImageLoading(false);
        setLoadError(error instanceof Error ? error.message : 'Could not generate this variation.');
      }
    })();
  ...
  }, [parsedData, seed, savedAudioUrl, savedImageUrl, user]);
  ...
  const handleDownload = async () => {
    ...
    try {
      const blob = generationAudioBlob.current ?? await (await fetch(audioUrl)).blob();
  ...
  useEffect(() => {
    const saveToSupabase = async () => {
      ...
      // Only upload fresh generations, not already-saved Supabase URLs
      const audioBlob = generationAudioBlob.current;
      const imageBlob = generationImageBlob.current;
