...
- **Two modes, one geometry.** The app is authored dark-first; light mode is the same UI, flat and calm: the poles swap at the token level in `src/stylesheet/light.css` (`text-white` → ink, `bg-black` → beige canvas, every hue ramp mirrored by the generated `light-palette.css`) and category rules there switch off blur, glow, gradients and glass edges wholesale — do not add light-mode exceptions per component. So: keep using `text-white` / `bg-white/5` / `border-white/10` for UI chrome — they re-theme themselves. In inline styles write `rgb(var(--tm-ink-rgb) / a)`, never a literal `rgba(255,255,255,a)`; see the header of `UniversalGlassKit.css` for the full set. A surface that must stay white in both modes (an iframe, a rendered image) uses the arbitrary value `bg-[#fff]`. Seasons are dark-only (plus `pureDark`, black with no gradient); light has one variant with a paper-warmth slider (white→cream) that `ThemeProvider` applies as inline variables on `<html>`. Mode, season and warmth persist in `localStorage` (`themeMode`, `seasonTheme`, `seasonPinned`, `lightWarmth`) — `src/themes/themeState.ts`. `SettingsModal` is imported statically on purpose: a lazy chunk suspends the route tree and flashes the spinner when it opens.
